...
{\let\newpage\relax\maketitle}
    ...
    \item Let $P$ be a $2 \times 2$ real orthogonal matrix and $\vec{x}$ is a real vector $\sbrak{x_1, x_2}^T$ with length $|\vec{x}| = \brak{x_1^2 + x_2^2}^{1/2}$. Then, which one of the following statements is correct?  
        ...
        \begin{enumerate}
            \item $|P\vec{x}| \leq |\vec{x}|$ where at least one vector satisfies $|P\vec{x}| \lt |\vec{x}|$
            \item $|P\vec{x}| = |\vec{x}|$ for all vectors $\vec{x}$
            \item $|P\vec{x}| \geq |\vec{x}|$ where at least one vector satisfies $|P\vec{x}| \gt |\vec{x}|$
